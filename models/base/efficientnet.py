...
class EfficientNet(OneStageClassifier):
    ...
    @staticmethod
    def B0(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B0(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B1(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B1(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B2(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B2(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B3(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B3(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B4(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B4(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B5(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = EfficientNetMain.B5(act=ACT.SILU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                       in_channels=in_channels)
        return EfficientNet(backbone=backbone, device=device, pack=pack, img_size=img_size, num_cls=num_cls)

    @staticmethod
    def B6(device=None, pack=None, num_cls=20, img_size=(224, 224), in_channels=3):
