...
class PolarV2Layer(PolarLayerProtyp):
    def __init__(self, in_channels, stride, num_cls, img_size=(256, 256), num_div=16, num_dstr=16,
                 act=ACT.RELU, norm=NORM.BATCH):
        super(PolarV2Layer, self).__init__(stride, num_cls, img_size=img_size, num_div=num_div)
        num_reg = num_dstr * num_div
        self.reg_dl = nn.Sequential(
            Ck3s1NA(in_channels=in_channels, out_channels=num_reg, act=act, norm=norm),
            Ck3s1NA(in_channels=num_reg, out_channels=num_reg, act=act, norm=norm),
    ...
    def decode(reg_dl, reg_chot, stride, xy_offset):
        xy_offset = xy_offset.to(reg_dl.device, non_blocking=True)
        reg_chot = reg_chot.permute(0, 2, 3, 1)
        reg_dl = reg_dl.permute(0, 2, 3, 1)
        Hf, Wf, _ = xy_offset.size()
        cens = ((xy_offset + 0.5) * stride).expand(reg_dl.size(0), Hf, Wf, 2)
        strides = torch.full(size=(reg_dl.size(0), Hf, Wf, 1), fill_value=stride, device=reg_dl.device)
        chot = torch.sigmoid(reg_chot)
        pred = torch.cat([cens, strides, reg_dl, chot], dim=-1)
...
class PolarV2ConstLayer(PolarLayerProtyp):
    def __init__(self, batch_size, stride, num_cls, img_size=(0, 0), num_div=36, num_dstr=16):
        super(PolarV2ConstLayer, self).__init__(stride, num_cls, img_size=img_size, num_div=num_div)
        self.feat = nn.Parameter(torch.zeros(batch_size, num_cls + num_div * num_dstr, self.Hf, self.Wf))
        init_sig(bias=self.feat[:, num_div * num_dstr:, :, :], prior_prob=0.001)
        self.num_dstr = num_dstr

    def forward(self, feat):
        reg_dl, reg_chot = self.feat.split((self.num_div * self.num_dstr, self.num_cls), dim=1)
        preds = PolarV2Layer.decode(
            reg_dl=reg_dl, reg_chot=reg_chot, stride=self.stride, xy_offset=self.xy_offset)
        return preds


class PolarV2ConstMain(nn.Module):
    def __init__(self, num_cls=80, img_size=(256, 256), batch_size=3, num_div=36, num_dstr=16):
        super(PolarV2ConstMain, self).__init__()
        self.num_cls = num_cls
        self.num_dstr = num_dstr
        self.img_size = img_size
        self.num_dstr = num_dstr
        self.layers = nn.ModuleList([
            PolarV2ConstLayer(batch_size=batch_size, stride=8, num_cls=num_cls, img_size=img_size,
                              num_div=num_div, num_dstr=num_dstr),
            PolarV2ConstLayer(batch_size=batch_size, stride=16, num_cls=num_cls, img_size=img_size,
                              num_div=num_div, num_dstr=num_dstr),
            PolarV2ConstLayer(batch_size=batch_size, stride=32, num_cls=num_cls, img_size=img_size,
                              num_div=num_div, num_dstr=num_dstr),
        ])
...
class PolarV2Main(DarkNetV8Bkbn, ImageONNXExportable):

    def __init__(self, Module, channelss, repeat_nums, num_cls=80, act=ACT.SILU, norm=NORM.BATCH,
                 img_size=(256, 256), in_channels=3, num_div=36, num_dstr=16):
        ...
        self.up = YoloV8UpStream(Module, down_channelss, up_channelss, repeat_num=repeat_nums[-1],
                                 act=act, norm=norm)
        self.layers = nn.ModuleList([
            PolarV2Layer(in_channels=channelss[1], stride=8, num_cls=num_cls, img_size=img_size,
                         num_div=num_div, num_dstr=num_dstr, act=act, norm=norm),
            PolarV2Layer(in_channels=channelss[2], stride=16, num_cls=num_cls, img_size=img_size,
                         num_div=num_div, num_dstr=num_dstr, act=act, norm=norm),
            PolarV2Layer(in_channels=channelss[3], stride=32, num_cls=num_cls, img_size=img_size,
                         num_div=num_div, num_dstr=num_dstr, act=act, norm=norm),
        ])
    ...
    @staticmethod
    def Nano(num_cls=80, act=ACT.SILU, norm=NORM.BATCH, img_size=(256, 256), in_channels=3, num_div=18, num_dstr=16):
        return PolarV2Main(**DarkNetV8Bkbn.PARA_NANO, num_cls=num_cls, act=act, norm=norm, img_size=img_size,
                           in_channels=in_channels, num_div=num_div, num_dstr=num_dstr)

    @staticmethod
    def Small(num_cls=80, act=ACT.SILU, norm=NORM.BATCH, img_size=(256, 256), in_channels=3, num_div=18, num_dstr=16):
        return PolarV2Main(**DarkNetV8Bkbn.PARA_SMALL, num_cls=num_cls, act=act, norm=norm, img_size=img_size,
                           in_channels=in_channels, num_div=num_div, num_dstr=num_dstr)

    @staticmethod
    def Medium(num_cls=80, act=ACT.SILU, norm=NORM.BATCH, img_size=(256, 256), in_channels=3, num_div=18, num_dstr=16):
        return PolarV2Main(**DarkNetV8Bkbn.PARA_MEDIUM, num_cls=num_cls, act=act, norm=norm, img_size=img_size,
                           in_channels=in_channels, num_div=num_div, num_dstr=num_dstr)

    @staticmethod
    def Large(num_cls=80, act=ACT.SILU, norm=NORM.BATCH, img_size=(256, 256), in_channels=3, num_div=18, num_dstr=16):
        return PolarV2Main(**DarkNetV8Bkbn.PARA_LARGE, num_cls=num_cls, act=act, norm=norm, img_size=img_size,
                           in_channels=in_channels, num_div=num_div, num_dstr=num_dstr)


class PolarV2(PolarV1):

    def __init__(self, backbone, device=None, pack=PACK.AUTO, radius=3.1, alpha=1, beta=6, max_match=10, **kwargs):
        super(PolarV2, self).__init__(backbone=backbone, device=device, pack=pack, radius=radius, alpha=alpha,
                                      beta=beta, max_match=max_match, )
    ...
    def imgs_tars2loss(self, imgs, targets, **kwargs):
        ...
        if inds_pos.size(0) > 0:
            ...
            inds_b_pos, inds_pos, cinds_tg, ious, dls_dstr_pd, dls_dstr_tg = \
                inds_b_pos[fltr_presv], inds_pos[fltr_presv], cinds_tg[fltr_presv], ious[fltr_presv], \
                dls_dstr_pd[fltr_presv], dls_dstr_tg[fltr_presv]
        ...
        return OrderedDict(cls=cls_loss * 0.5, iou=iou_loss * 7.5, dfl=dfl_loss * 1.5)
    ...
    def Nano(device=None, num_cls=80, img_size=(416, 352), pack=PACK.AUTO, num_div=18, num_dstr=16):
        backbone = PolarV2Main.Nano(num_cls=num_cls, act=ACT.SILU, img_size=img_size, num_div=num_div,
                                    num_dstr=num_dstr)
        return PolarV2(backbone=backbone, device=device, pack=pack)
    ...
    def Const(device=None, num_cls=80, img_size=(416, 352), batch_size=1, num_div=18, num_dstr=16):
        backbone = PolarV2ConstMain(num_cls=num_cls, batch_size=batch_size, img_size=img_size, num_div=num_div,
                                    num_dstr=num_dstr)
        return PolarV2(backbone=backbone, device=device, pack=PACK.NONE)


if __name__ == '__main__':
    model = PolarV2.Medium(img_size=(512, 512), num_cls=20)
